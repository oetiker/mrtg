...
and go into F<c:\mrtg-2.16.2\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.16.2\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.16.2\bin> directory and typing:
...
Copy B<srvany.exe> and B<instsrv.exe> to c:\mrtg-2.16.2\bin\ (your MRTG bin directory).
