...
mrtg-reference - MRTG 2.15.1 configuration reference
...
present (see the L<mrtg-ipv6> manpage). When IPv6 is enabled, mrtg can talk to routers
...
Example:
 ...
 Target[myrouter]: .1.3.6.1.4.1.9.10.117.1.1.4.1.10.IndexPOS.1/1.2.3.4&.1.3.6.1.4.1.9.10.117.1.1.4.1.11.IndexPOS.1/1.2.3.4:public@myrouter
...
As well as the I<Directory> option requires, the clone directory must already exist; mrtg will not create it.
...
broken and un-broken messages get sent in close succession, we only send an unbroken message once the curent value is 0.1 (10%) away from the threshold.
