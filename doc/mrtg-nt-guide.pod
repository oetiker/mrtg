...
mrtg-nt-guide - The Windows Guide to MRTG 2.14.1
...
http://oss.oetiker.ch/mrtg/pub Look for mrtg-2.14.1.zip
...
Unzip MRTG to C:\mrtg-2.14.1 on the Windows machine of your choice.
...
and go into F<c:\mrtg-2.14.1\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.14.1\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.14.1\bin> directory and typing:
...
to your mrtg.cfg file and start it with:
 ...
 start /Dc:\mrtg-2.14.1\bin wperl mrtg --logging=eventlog mrtg.cfg
