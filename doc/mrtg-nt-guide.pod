...
mrtg-nt-guide - The Windows Guide to MRTG 2.11.1
...
http://people.ee.ethz.ch/~oetiker/webtools/mrtg/pub Look for mrtg-2.11.1.zip
...
and go into F<c:\mrtg-2.11.1\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.11.1\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.11.1\bin> directory and typing:
...
this HOWTO you B<MUST> obtain both executables.
...
PsTools from SysInternals.  There are only two executables that are needed
here from the PsTools package - pslist.exe to list running processes and
pskill.exe to kill running processes.  For more information about PsTools
please visit L<http://www.sysinternals.com/ntw2k/freeware/pstools.shtml>.
Please note that PsTools are B<optional>, but helpful to have more control
over the MRTG processes (or any other system processes for that matter).
...
Create a file called B<mrtg.bat> in c:\mrtg\bin\ (your MRTG bin directory). 
This file should include the command line you use to start MRTG, for
instance:
...
following content into it:
 ...
 "ImagePath"=hex(2):63,00,3a,00,5c,00,6d,00,72,00,74,00,67,00,5c,00,62,00,69,00,\
   6e,00,5c,00,73,00,72,00,76,00,61,00,6e,00,79,00,2e,00,65,00,78,00,65,00,00,\
   00
 "DisplayName"="MRTG"
...
Optional: Copy pslist.exe and pskill.exe to c:\windows\system32\ or any other executable path.
...
Now you need to create the registry entry required for the new service.  You
...
key.  Under Parameters make sure that the B<Application> variable is
correctly pointing to the B<mrtg.bat> script located in your MRTG bin
directory.
...
and perl.exe.  Make sure to stop any previously running MRTG processes to
avoid confict.

=head2 Caveats

I am aware of only one problem with this setup - you cannot simply stop the
MRTG service using the Management Console.  The 'perl' process continues to
run and has to be killed manually using the PsTools included in this package
- pslist and pskill.  This is due to the fact that the SRVANY tools were
developed to work only with executable files, and not batch scritps.
...
Tobi Oetiker E<lt>oetiker@ee.ethz.chE<gt>,
