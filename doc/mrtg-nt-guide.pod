...
and go into F<c:\mrtg-2.10.15\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.10.15\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.10.15\bin> directory and typing:
