...
mrtg-reference - MRTG 2.16.1 configuration reference
...
If you want to use a special OID for queriing the router uptime,
use prepend the oid.
...
As well as the I<Directory> option requires, the clone directory must
already exist; mrtg will not create it.
...
NOTE: the Clonedirectory option must always be 'relative' or bad things will
happen.
...
broken and un-broken messages get sent in close succession, we only send an
...
=head2 HW Failure Bassed Threshol Checking
