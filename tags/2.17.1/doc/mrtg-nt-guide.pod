...
mrtg-nt-guide - The MRTG 2.17.1 Windows Installation Guide
...
mrtg-2.17.1.zip or better. The archive also contains a precompiled copy of
...
Unzip MRTG to C:\mrtg-2.17.1 on the Windows machine of your choice.
...
and go into F<c:\mrtg-2.17.1\bin>. Type:
...
Get to a cmd prompt and change to the F<c:\mrtg-2.17.1\bin> directory. Type the
following command:
...
results by going into the F<c:\mrtg-2.17.1\bin> directory and typing:
...
to your mrtg.cfg file and start it with:
 ...
 start /Dc:\mrtg-2.17.1\bin wperl mrtg --logging=eventlog mrtg.cfg
...
Copy B<srvany.exe> and B<instsrv.exe> to c:\mrtg-2.17.1\bin\ (your MRTG bin directory).
